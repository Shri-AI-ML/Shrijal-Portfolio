...
import { Cpu, Code, Database, Globe, Sliders, CheckCircle } from 'lucide-react';
...
const Skills = () => {
  ...
  const skillCategories = [
    {
      ...
      icon: Cpu,
      iconColor: "text-purple-400 border-purple-500/20 bg-purple-950/10",
      skills: [
    ...
    },
    {
      ...
      icon: Sliders,
      iconColor: "text-indigo-400 border-indigo-500/20 bg-indigo-950/10",
      skills: [
    ...
    },
    {
      ...
      icon: Code,
      iconColor: "text-emerald-400 border-emerald-500/20 bg-emerald-950/10",
      skills: [
    ...
    },
    {
      ...
      icon: Database,
      iconColor: "text-blue-400 border-blue-500/20 bg-blue-950/10",
      skills: [
  ...
  return (
    <section id="skills" className="py-16 md:py-24 border-t border-zinc-900 relative">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {skillCategories.map((category, catIdx) => {
            ...
            return (
              <div 
                key={catIdx} 
                className="glass-panel border border-zinc-900/80 rounded-xl p-6 hover:border-zinc-800/80 transition-all flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center gap-3 mb-6">
                    <div className={`p-2 border rounded-lg ${category.iconColor}`}>
                  ...
                  <div className="flex flex-wrap gap-2.5">
                    {category.skills.map((skill, skIdx) => {
                      ...
                      return (
                        ...
                        >
                          <span 
                            className={`inline-flex items-center gap-1.5 text-xs font-mono px-3.5 py-2 rounded-lg border transition-all duration-300 cursor-help select-none ${
                              isHovered 
                                ? 'bg-purple-950/20 border-purple-500/50 text-purple-300 shadow-[0_0_15px_rgba(139,92,246,0.15)] scale-102' 
                                : 'bg-zinc-950 border-zinc-900 text-zinc-400 hover:border-zinc-700 hover:text-zinc-200'
                            }`}
                          >
                            <span className={`h-1.5 w-1.5 rounded-full ${isHovered ? 'bg-purple-400' : 'bg-zinc-700'}`}></span>
                            {skill.name}
                          </span>
                        </div>
                      );
                ...
                {/* Info Box detailing hovered skill */}
                <div className="mt-8 pt-4 border-t border-zinc-900/60 min-h-[56px] flex items-center">
                  {hoveredSkill && hoveredSkill.startsWith(`${catIdx}-`) ? (
                    <div className="flex gap-2.5 items-start text-xs text-zinc-400 font-mono animate-fadeIn">
                    ...
                    <span className="text-xs font-mono text-zinc-600 italic">
                      // Hover over a skill node for details
                    </span>
                  )}
                </div>
